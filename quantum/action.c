...
#ifndef NO_ACTION_ONESHOT
        if (is_oneshot_layer_active() && record->event.pressed && !keymap_config.oneshot_disable && should_clear_oneshot_layer(record, false)) {
            clear_oneshot_layer_state(ONESHOT_OTHER_KEY_PRESSED);
        }
#endif
        return;
...
void process_action(keyrecord_t *record, action_t action) {
    ...
    if (is_oneshot_layer_active() && event.pressed && !keymap_config.oneshot_disable
