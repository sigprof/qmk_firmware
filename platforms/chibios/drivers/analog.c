...
#if STM32_ADCV3_OVERSAMPLING
#    if !defined(ADC_OVERSAMPLING_RATIO_BITS)
#        define ADC_OVERSAMPLING_RATIO_BITS 0
#    elif (ADC_OVERSAMPLING_RATIO_BITS < 0) || (ADC_OVERSAMPLING_RATIO_BITS > 8)
#        error "ADC_OVERSAMPLING_RATIO_BITS must be from 0 to 8"
#    endif
#    if !defined(ADC_OVERSAMPLING_SHIFT_BITS)
#        define ADC_OVERSAMPLING_SHIFT_BITS ADC_OVERSAMPLING_RATIO_BITS
#    elif (ADC_OVERSAMPLING_SHIFT_BITS < 0) || (ADC_OVERSAMPLING_SHIFT_BITS > 8)
#        error "ADC_OVERSAMPLING_SHIFT_BITS must be from 0 to 8"
#    endif
#    if (ADC_OVERSAMPLING_RATIO_BITS == 0) && (ADC_OVERSAMPLING_SHIFT_BITS > 0)
#        error "Cannot use ADC_OVERSAMPLING_SHIFT_BITS without ADC_OVERSAMPLING_RATIO_BITS"
#    elif ADC_OVERSAMPLING_RATIO_BITS > ADC_OVERSAMPLING_SHIFT_BITS + 4
// Not an error, because the bit count also depends on ADC_RESOLUTION, which is
// hard to check due to its value being not just the number of bits.
#        pragma message "ADC_OVERSAMPLING_RATIO_BITS and ADC_OVERSAMPLING_SHIFT_BITS values may cause overflow"
#    endif
#else
#    if ADC_OVERSAMPLING_RATIO_BITS > 0
#        error "ADC_OVERSAMPLING_RATIO_BITS is set, but the MCU does not support oversampling"
#    endif
#    if ADC_OVERSAMPLING_SHIFT_BITS > 0
#        error "ADC_OVERSAMPLING_SHIFT_BITS is set, but the MCU does not support oversampling"
#    endif
#endif
...
#    if STM32_ADCV3_OVERSAMPLING && (ADC_OVERSAMPLING_RATIO_BITS > 0)
    .cfgr2 = ADC_CFGR2_ROVSE | (((ADC_OVERSAMPLING_RATIO_BITS)-1) << ADC_CFGR2_OVSR_Pos) | ((ADC_OVERSAMPLING_SHIFT_BITS) << ADC_CFGR2_OVSS_Pos),
#    endif
